...
window.selectRole = function(role) {
    // Lấy thẻ ảnh đang hiển thị
    const imgElement = document.getElementById('hero-img');

    // --- XỬ LÝ CHỌN ẢNH VÀ XƯNG HÔ ---

    // 1. TRƯỜNG HỢP: BẠN BÈ (Dùng ảnh 3 như bạn yêu cầu)
    if (role === 'friend') {
        imgElement.src = 'anh-3.jpg';  // <--- Ảnh 3 cho Bạn
        
        pronouns.me = 'Thơ';  // Hoặc tên của bạn
        pronouns.you = 'Bạn';
    } 
    
    // 2. TRƯỜNG HỢP: NGƯỜI XEM LÀ EM (Mình là Chị)
    else if (role === 'younger') {
        imgElement.src = 'anh-1.jpg';  // Dùng ảnh 1 cho vai Chị
        
        pronouns.me = 'Chị';
        pronouns.you = 'Em';
    } 
    
    // 3. TRƯỜNG HỢP: NGƯỜI XEM LÀ ANH/CHỊ (Mình là Em)
    else {
        imgElement.src = 'anh-2.jpg';  // Dùng ảnh 2 cho vai Em
        
        if (role === 'older-male') {
            pronouns.me = 'Em'; pronouns.you = 'Anh';
        } else {
            // older-female
            pronouns.me = 'Em'; pronouns.you = 'Chị';
        }
    }

    // Chuyển sang màn hình mở thiệp
    goToEnvelope();
};
